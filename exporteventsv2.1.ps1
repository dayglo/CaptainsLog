...
function GetInfo ($cs , $INFRServer) {
	...
	if ($events) {remove-variable events}
	...
	Catch { Write-Host "Error in get VIEvent, exiting Getinfo function." 
			...
			#enter a default line to say that we couldnt contact the infrastructure
			$ClusID = AddCluster $cs "n/a" $EnvID
			$ServerID = AddServer $cs "VirtualCenter" $ClusID
			$message = "Could not contact the infrastructure - $err"

			addEventsToDB $INFRServer $ClusID $ServerID $now $message 1 1
	...
	if ($events) {
		...
		foreach ($event in $sortedevents) {
			...
			if (($event.FullFormattedMessage) -and ($event.CreatedTime)) {
				...
				if (!$event.Cluster) {$event.Cluster = "n/a"}
				if (!$event.Host) {$event.Host = "VirtualCenter"} 
				
	 			addEventsToDB $INFRServer $event.Cluster $event.Host $event.CreatedTime $event.FullFormattedMessage 1 $event.NumOccurrences
			}
		}

		} else {
			
			#there were no events returned. do nothing.
	...
	if ($nextCollectionTime -gt $now ) {
		...
		If (($hostsalarms | Measure-Object).count -gt 0) {
            ...
            foreach ($hostalarm in $hostsalarms){
                ...
                addEventsToDB $INFRServer $null $hostalarm.Object $now $alarmstring 2 1
			}
		}   
	...
	if ($nextCollectionTime -gt $now ) {
		...
		If (($datastoresalarms | Measure-Object).count -gt 0) {
            ...
            foreach ($datastorealarm in $datastoresalarms){
                ...
                addEventsToDB $INFRServer $null $datastorealarm.Object $now $alarmstring 3 1
			}
		}
	}
...
#================================================================================================
function addEventsToDB($env,$clus,$obj,$time,$content,$type,$occurrences){
        ...
        if (!$env) {
			...
			$SQLParameter5 = New-Object System.Data.SqlClient.SqlParameter("@occurences", $occurences)
